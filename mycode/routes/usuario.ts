...
function encriptarTexto(plainText,secretKey){
    const iv=CryptpJS.lib.WordArray.random(16);
    const bytesEncriptados=CryptpJS.AES.encrypt(plainText,secretKey,{
    iv:iv,
    mode:CryptpJS.mode.CBC,
    padding:CryptpJS.pad.Pkcs7
    });
...
//Funcion para el cdescencifrado
function descencriptarTexto(encryptedText,vector_iv,secretKey){
    const iv=CryptpJS.enc.Hex.parse(vector_iv);
    const decifrado=CryptpJS.AES.decrypt(encryptedText,secretKey,{
        iv:iv,
        mode:CryptpJS.mode.CBC,
        padding:CryptpJS.pad.Pkcs7
    });
    return decifrado.toString(CryptpJS.enc.Utf8);
